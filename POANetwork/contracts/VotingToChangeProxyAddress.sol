...
//@audit - This contract defines the voting mechanism required to change the addresses of multiple contracts stored in ProxyStorage
contract VotingToChangeProxyAddress {
    using SafeMath for uint256;
    //@audit - Enum representing the current state of quorum for a ballot
    enum QuorumStates {Invalid, InProgress, Accepted, Rejected}
    //@audit - Enum representing the different choices made for a vote on a ballot
    enum ActionChoice { Invalid, Accept, Reject }

    //@audit - The ProxyStorage contract address, cast to an interface
    IProxyStorage public proxyStorage;
    //@audit - The maximum number of old mining keys to check when testing for old votes
    uint8 public maxOldMiningKeysDeepCheck = 25;
    //@audit - The uint ID of the next ballot that will be created
    uint256 public nextBallotId;
    //@audit - A dynamic array of currently active ballots
    uint256[] public activeBallots;
    //@audit - The length of the activeBallots array
    uint256 public activeBallotsLength;

    //@audit - Struct representing voting data for a ballot
    struct VotingData {
        uint256 startTime;
        uint256 endTime;
        uint256 totalVoters;
    ...
    //@audit - Maps uint ballot IDs to their VotingData structs
    ...
    //@audit - Constructor: Sets the ProxyStorage contract address, cast to an interface
    function VotingToChangeProxyAddress(address _proxyStorage) public {
        proxyStorage = IProxyStorage(_proxyStorage);
    }

    //@audit - Allows a validator to create a ballot to change various addresses stored in ProxyStorage
    ...
    function createBallotToChangeProxyAddress(
        ...
        //@audit - Ensure the given start and end times are not 0
        ...
        //@audit - Ensure the endtime comes after the starttime, and that the starttime is after now
        require(_endTime > _startTime && _startTime > getTime());
        //@audit - Ensure that the proposed address change is not 0
        require(_proposedValue != address(0));activeBallots
        //@audit - Create the VotingData struct for this ballot
        ...
        //@audit - Set the ballot data in the votingState mapping, using the nextBallotId as the index
        votingState[nextBallotId] = data;
        //@audit - Push the nextBallotId to the end of the activeBallots array
        activeBallots.push(nextBallotId);
        //@audit - Update activeBallotsLength
        activeBallotsLength = activeBallots.length;
        //@audit - Emit a BallotCreated event
        BallotCreated(nextBallotId, 5, msg.sender);
        //@audit - increment the nextBallotId
        nextBallotId++;
    }
    ...
    function vote(uint256 _id, uint8 _choice) public onlyValidVotingKey(msg.sender) {
        ...
        //@audit - Get the mining key associated with the sender via the KeysManager contract
        address miningKey = getMiningByVotingKey(msg.sender);
        //@audit - Ensure the ballot can be voted on and that the sender can vote on the ballot
        //@audit - NOTE: Place this first in the function, so it fails quickly
        require(isValidVote(_id, msg.sender));
        //@audit - If the selected choice is ActionChoice.Accept
        if (_choice == uint(ActionChoice.Accept)) {
            //@audit - Increment ballot progress
            ballot.progress++;
        //@audit - If the selected choice is ActionChoice.Reject
        } else if (_choice == uint(ActionChoice.Reject)) {
            //@audit - Decrement ballot progress
            ballot.progress--;
        } else {
            //@audit - If an invalid choice is made, revert
            revert();
        }
        //@audit - Increment total voters for a ballot
        ballot.totalVoters++;
        //@audit - Set the sender's key as having voted on this ballot
    ...
    //@audit - Get the proposed address change from the ballot
    function getProposedValue(uint256 _id) public view returns(address) {
        return votingState[_id].proposedValue;
    }

    //@audit - Get the type of contract for the ballot
    function getContractType(uint256 _id) public view returns(uint256) {
        return votingState[_id].contractType;
    }

    //@audit - Get the global minimum threshol of voters required to finalize a ballot from the BallotsStorage contract
    function getGlobalMinThresholdOfVoters() public view returns(uint256) {
        ...
        //@audit - Get the threshold of voters and return it
        return ballotsStorage.getProxyThreshold();
    }

    //@audit - Get the progress toward a referenced ballot
    function getProgress(uint256 _id) public view returns(int) {
        return votingState[_id].progress;
    }

    //@audit - Get the total number of voters that have voted on a referened ballot
    function getTotalVoters(uint256 _id) public view returns(uint256) {
        return votingState[_id].totalVoters;
    }

    //@audit - Returns the minimum threshold of voters required to finalize a given ballot
    function getMinThresholdOfVoters(uint256 _id) public view returns(uint256) {
        return votingState[_id].minThresholdOfVoters;
    }

    //@audit - Returns the mining key address associated with the passed-in voting key
    function getMiningByVotingKey(address _votingKey) public view returns(address) {
        //@audit - Get the KeysManager contract address, and cast it to an interface
        IKeysManager keysManager = IKeysManager(getKeysManager());
        //@audit - Get the mining key from the KeysManager contract and return it
        return keysManager.getMiningKeyByVoting(_votingKey);
    }

    //@audit - Get the BallotsStorage contract address from the ProxyStorage contract
    function getBallotsStorage() public view returns(address) {
        return proxyStorage.getBallotsStorage();
    }

    //@audit - Get the KeysManager contract address from the ProxyStorage contract
    function getKeysManager() public view returns(address) {
        return proxyStorage.getKeysManager();
    }

    //@audit - Get the uint start time of a referenced ballot
    function getStartTime(uint256 _id) public view returns(uint256) {
        return votingState[_id].startTime;
    }

    //@audit - Get the uint end time of a referenced ballot
    function getEndTime(uint256 _id) public view returns(uint256) {
        return votingState[_id].endTime;
    }

    //@audit - Return true if the referenced ballot is finalized
    ...
    function hasAlreadyVoted(uint256 _id, address _votingKey) public view returns(bool) {
        //@audit - Get the ballot referenced by the id
        //@audit - NOTE: This should be memory, as the VotingData struct is not being modified
        VotingData storage ballot = votingState[_id];
    ...
    function areOldMiningKeysVoted(uint256 _id, address _miningKey) public view returns(bool) {
        ...
        IKeysManager keysManager = IKeysManager(getKeysManager());
        //@audit - Iterates over old mining keys and checks to make sure they have not voted on the ballot
    ...
    function finalizeBallot(uint256 _id) private {
        //@audit - If the ballot progress is nonzero (that is, at least one net positive vote has been cast), and the total voters in the ballot is above the threshold for this ballot
        if (getProgress(_id) > 0 && getTotalVoters(_id) >= getMinThresholdOfVoters(_id)) {
            //@audit - Update the ballot with the QuorumStates.Accepted state. See updateBallot for more detailed notes
            updateBallot(_id, uint8(QuorumStates.Accepted));
            //@audit - Set the new contract address in the ProxyStorage contract
            proxyStorage.setContractAddress(getContractType(_id), getProposedValue(_id));
        } else {
            //@audit - Update the ballot with the "rejected" state
            updateBallot(_id, uint8(QuorumStates.Rejected));
        }
        //@audit - Finally, deactivate the ballot. See the function deactiveBallot for more detailed notes.
        deactiveBallot(_id);
    }
